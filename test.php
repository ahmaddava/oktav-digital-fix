...
class ProductionCalculator extends Page implements HasForms
{
    ...
    public function getFloatVal($value): float
    {
    ...
    protected function _calculateFinalPriceInternal(array &$allDataArray, bool $showNotification = true): void
    {
        ...
        if ($quantity <= 0) {
            $quantity = 1; // Mencegah pembagian dengan nol
        }
        $currentBoxType = $allData['box_type_selection'] ?? null;

        // 1. Hitung total biaya material per unit
        ...
        // 2. Ambil semua rate biaya dari database
        $masterCostData = !empty($allData['size']) ? MasterCost::where('size', $allData['size'])->first() : null;
        $polyCostData = !empty($allData['poly_dimension']) ? PolyCost::where('dimension', $allData['poly_dimension'])->first() : null;

        // Ambil rate sesuai jenis biayanya
        $productionCostPerItem = $masterCostData ? $this->getFloatVal($masterCostData->production_cost ?? 0) : 0.0;
        $batchKnifeCost = $masterCostData ? $this->getFloatVal($masterCostData->knife_cost ?? 0) : 0.0;
        $batchPolyCost = $polyCostData ? $this->getFloatVal($polyCostData->cost ?? 0) : 0.0;
        $masterCostProfitPercentage = $masterCostData ? $this->getFloatVal($masterCostData->profit ?? 0) : 0.0;

        // 3. Pisahkan biaya menjadi variabel (per item) dan tetap (per batch)
        $totalVariableCost = ($totalMaterialCostPerUnit + $productionCostPerItem) * $quantity;

        $totalFixedCost = 0.0;
        if ($polyCostData) {
            $totalFixedCost += $batchPolyCost;
        }
        if (($allData['include_knife_cost'] ?? 'tidak_ada') === 'ada' && $masterCostData) {
            $totalFixedCost += $batchKnifeCost;
        }

        // 4. Hitung total biaya sebelum profit
        $finalTotalCostBeforeProfit = $totalVariableCost + $totalFixedCost;

        // 5. Kalkulasi Profit
        $appliedProfitPercentage = $masterCostProfitPercentage;
        ...
        // 8. Simpan ringkasan biaya untuk TAMPILAN (VIEW)
        $this->summaryTotalMaterialCost = $totalMaterialCostPerUnit;
        $this->summaryTotalProductionWorkCost = $productionCostPerItem;
        $this->summaryTotalPolyCost = ($quantity > 0) ? ($batchPolyCost / $quantity) : 0;
        $this->summaryActualKnifeCost = ($quantity > 0) ? ($batchKnifeCost / $quantity) : 0;
        $this->summaryProfitAmount = ($quantity > 0) ? ($totalProfitAmount / $quantity) : 0;

        $this->summarySellingPricePerItem = ($quantity > 0) ? ($finalTotalCost / $quantity) : 0;
        $this->summaryTotalPrice = $finalTotalCost;
        $this->displayAppliedProfitPercentage = $appliedProfitPercentage;

        $this->calculationResult = "Rp " . number_format($finalTotalCost, 0, ',', '.');

        // 9. Simpan data kalkulasi ke array (untuk disimpan ke DB)
        $allDataArray['total_material_cost_summary'] = $totalVariableCost;
        $allDataArray['production_cost_summary'] = $productionCostPerItem * $quantity;
        $allDataArray['poly_cost_summary'] = $batchPolyCost;
        $allDataArray['knife_cost_summary'] = $batchKnifeCost;
        $allDataArray['profit_amount_summary'] = $totalProfitAmount;
        $allDataArray['total_price_per_item_summary'] = $this->summarySellingPricePerItem;
        ...
        if ($showNotification) {
            ...
            Notification::make()
                ...
                ->body('Total Estimasi: ' . $this->calculationResult . ' (Harga per Item: Rp ' . number_format($this->summarySellingPricePerItem, 0, ',', '.') . ')')
                ->success()
                ->send();
        }
    }
    ...
    {
        $dimensionInputSchema = function (string $name, string|\Closure $label, bool $isBawah = false): TextInput {
            ...
            if ($isBawah) {
                $input = $input->afterStateUpdated(function (Get $get, Set $set, $state, $livewire) {
                    if ($get('box_type_selection') === 'TAB' && is_numeric($state)) {
                        $set('atas_panjang', $livewire->getFloatVal($get('bawah_panjang')) + 0.5);
                        $set('atas_lebar', $livewire->getFloatVal($get('bawah_lebar')) + 0.5);
                        $set('atas_tinggi', $livewire->getFloatVal($get('bawah_tinggi')) + 0.5);
                    }
                    $livewire->updateAllCalculations();
                });
            } else {
                $input = $input->afterStateUpdated(fn($livewire) => $livewire->updateAllCalculations());
            }
        ...
        $sections = [
            ...
            Section::make('Dimensi Box Input')
                ...
                ->columns(2)
                ->schema([
                    Fieldset::make('fieldset_dimensi_bawah')
                        ->label('Dimensi Box Bawah')
                        ->columns(1)
                        ->hidden(fn(Get $get): bool => !$isPartVisible($get, 'bawah'))
                        ->schema([
                            $dimensionInputSchema('bawah_panjang', 'Panjang', true),
                            $dimensionInputSchema('bawah_lebar', 'Lebar', true),
                            $dimensionInputSchema('bawah_tinggi', 'Tinggi', true),
                        ]),
                    Fieldset::make('fieldset_dimensi_atas')
                        ->label(fn(Get $get) => "Dimensi " . $getDynamicLabel($get, "atas"))
                        ->columns(1)
                        ->hidden(fn(Get $get): bool => !$isPartVisible($get, 'atas'))
                        ->schema([
                            $dimensionInputSchema('atas_panjang', 'Panjang', false),
                            $dimensionInputSchema('atas_lebar', 'Lebar', false),
                            $dimensionInputSchema('atas_tinggi', 'Tinggi', false),
                        ]),
                ]),
            ...
            Section::make('Hasil Perhitungan Dimensi Komponen (Potongan Jadi)')
                ->columns(1)->collapsible()->collapsed()->icon('heroicon-o-clipboard-document-list')
                ->schema([
            ...
            Section::make('Hasil Perhitungan Kuantitas dari Bahan')
                ->columns(1)->collapsible()->collapsed()->icon('heroicon-o-view-columns')
                ->schema([
