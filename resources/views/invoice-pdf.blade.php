...
<body class="px-2 py-3">
    <div class="max-w-6xl mx-auto  p-4">
        <div class="flex justify-between items-start mb-[-20px]">
            <div class="mt-4">
                <img alt="Oktav Printing logo" class="mb-1" src="{{ asset('images/oktav.jpg') }}" width="100"
                    height="50" />
            </div>
                ...
                <table class="border-collapse">
                    <tbody>
                        <tr class="border-b border-gray-400">
                            <td colspan="2" class="pb-1 font-semibold">No. {{ $invoice->invoice_number }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td colspan="2" class="pt-1">{{ $invoice->created_at->format('d M Y') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <table class="w-full mt-1 border-collapse">
            ...
            <tbody>
                @foreach ($invoice->invoiceProducts->chunk(5) as $page => $itemsChunk)
                    <div class="page">
                        <!-- Ganti variabel $productsChunk menjadi $itemsChunk dan $product menjadi $item -->
                        @foreach ($itemsChunk as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                                <td class="py-2 px-4 text-sm text-gray-700 text-center">
                                    {{ $page * 5 + $loop->iteration }}</td>

                                <!-- ✅ Gunakan logika yang sama seperti di accessor untuk mendapatkan nama -->
                                <td class="py-2 px-4 text-sm text-gray-700 font-medium">
                                ...
                                <td class="py-2 px-1">
                                    <div class="flex justify-end items-center gap-0 text-sm text-gray-700">
                                        ...
                                        <!-- ✅ Gunakan $item->price, bukan $product->price -->
                                        <span
                                            class="w-24 text-right">{{ number_format($item->price, 0, ',', '.') }}</span>
                                    </div>
                                </td>

                                <!-- ✅ Gunakan $item->quantity, bukan $product->pivot->quantity -->
                                <td class="py-2 px-4 text-sm text-gray-700 text-right">{{ $item->quantity }}</td>

                                <td class="py-2 px-4">
                                    <div class="flex justify-end items-center gap-0 text-sm text-gray-700">
                                        <span class="w-6 text-right tracking-tighter">Rp</span>
                                        <!-- ✅ Gunakan $item->total_price atau hitung ulang -->
                                        <span
                                            class="w-24 text-right font-medium">{{ number_format($item->total_price, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                            </tr>
                        @endforeach

                        <!-- Baris Kosong (Logika ini tetap sama) -->
                        @for ($i = $itemsChunk->count(); $i < 5; $i++)
                            <tr class="border-b border-gray-200" style="height: 36px;">
                                <!-- Beri tinggi agar konsisten -->
                                <td>&nbsp;</td>
                                <td></td>
                                <td></td>
                                <td></td>
            ...
            <table class="w-full">
                <tr>
                    ...
                    <td class="align-top w-2/5">
                        ...
                        <div class="mt-4">
                            <p class="mb-[90px]">Pelanggan,</p>
                            <!-- Increased margin-bottom to push "Pelanggan" further down -->
                            <p class="mt-2 font-bold">{{ $invoice->name_customer }}</p>
                            <!-- Reduced margin-top to bring "Bu Syifa" closer to the next element -->
                        </div>
                    </td>
